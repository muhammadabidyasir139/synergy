...
export default function AIInsightPage() {
  ...
  useEffect(() => { loadRecommendations(activeProfile); }, [activeProfile]);
  ...
  return (
    <div className={styles.page}>
        ...
        <div className={styles.profileBtns}>
          {(["conservative", "balanced", "aggressive"] as Profile[]).map((p) => (
            <button key={p}
              ...
              <span className={styles.profileLabel}>{p === "conservative" ? "Konservatif" : p === "balanced" ? "Balanced" : "Agresif"}</span>
            </button>
          ))}
        </div>
      ...
      <div className={styles.statsRow}>
        <div className={styles.statCard}><span className={styles.statIcon}>📊</span><div><p className={styles.statVal}>50.000+</p><p className={styles.statLabel}>Data Training XGBoost</p></div></div>
        <div className={styles.statCard}><span className={styles.statIcon}>🎯</span><div><p className={styles.statVal}>94.2%</p><p className={styles.statLabel}>Akurasi Prediksi Model</p></div></div>
        <div className={styles.statCard}><span className={styles.statIcon}>🔄</span><div><p className={styles.statVal}>Real-time</p><p className={styles.statLabel}>Update Scoring Harian</p></div></div>
      </div>

      <div>
        <h3 className={styles.sectionTitle}>
          Rekomendasi untuk Anda {!isLoading && `(${recommendations.length} UMKM)`}
        </h3>
        {isLoading ? (
          <p style={{ padding: "1rem", color: "var(--text-muted)" }}>Memuat rekomendasi AI...</p>
        ) : (
            ...
            {recommendations.map((umkm) => (
              <div key={umkm.id} className={styles.recCard}>
                ...
                <div className={styles.scoreBar}>
                  <div className={`${styles.scoreFill} ${umkm.aiScore >= 75 ? styles.fillGreen : umkm.aiScore >= 55 ? styles.fillYellow : styles.fillRed}`}
                ...
                <div className={styles.recMetrics}>
                  <div className={styles.recMetric}><span className={styles.mLabel}>Est. Profit</span><span className={styles.mVal}>{umkm.estimatedProfit}</span></div>
                  <div className={styles.recMetric}><span className={styles.mLabel}>ROI</span><span className={styles.mValGreen}>{umkm.roi}</span></div>
                  <div className={styles.recMetric}>
                    <span className={styles.mLabel}>Risk Level</span>
                    <span className={`${styles.mVal} ${umkm.risk === "LOW" ? styles.colorGreen : umkm.risk === "MEDIUM" ? styles.colorYellow : styles.colorRed}`}>{umkm.risk}</span>
                  </div>
                </div>

                <Link href={`/investor/dashboard/investasi?campaignId=${umkm.id}`} className={styles.investBtn}>
                  Investasi Sekarang
                </Link>
              </div>
            ))}
            {recommendations.length === 0 && <p style={{ padding: "1rem", color: "var(--text-muted)" }}>Tidak ada rekomendasi untuk profil ini.</p>}
          </div>
        )}
      </div>
    </div>
