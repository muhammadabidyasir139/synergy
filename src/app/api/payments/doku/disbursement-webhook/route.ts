...
const REQUEST_TARGET = "/api/payments/doku/disbursement-webhook";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("[DOKU disbursement-webhook] not configured", err);
  ...
  const invoiceNumber: string | undefined = body?.disbursement?.invoice_number;
  const dokuStatus: string | undefined = body?.disbursement?.status ?? body?.transaction?.status;
  if (!invoiceNumber) {
  ...
  const status = dokuStatus === "SUCCESS" || dokuStatus === "COMPLETED" ? "PAID" : "FAILED";
